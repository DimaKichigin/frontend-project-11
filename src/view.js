...
export const renderText = (i18n, elems) => {
  ...
  elems.createdBy.innerHTML = i18n.t('createdBy')
    + i18n.t(
      ' <a href="https://github.com/DimaKichigin" target="_blank"  >Dima Kichigin</a>',
    );
};
...
const renderMessage = (elems, message, i18n) => {
  elems.feedback.textContent = i18n.t([
    `messages.${message}`,
    'messages.default',
  ]);
};
...
//    const render = (watchedState, elements, i18Instance) => {
//       formEl.reset();
//       inputEl.focus();
...
//    // удаление все из контейнера и перерисовка всего заново
//       const buttons = state.posts.map(item => {
...
//           console.log(state.uiState.selectedCompanyId)
//           const nextSelectedId = state.uiState.selectedCompanyId === item.id ? null : item.id
...
//    //    render(watchedState, elements, i18Instance)
//    //       })
//    //    return watchedState
//          }
